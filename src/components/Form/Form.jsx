...
export const FormEl = ({ myFunction }) => {
  ...
  const handleInputChange = event => {
    ...
    setValue(password);
    const lettersLow = 'qwertyuiopasdfghjklzxcvbnmQWERTYUIOPLKJHGFDSAZXCVBNM';
    ...
    for (let i = 0; i < password.length; i++) {
      if (lettersLow.includes(password[i])) {
        isLeters = 'true';
    ...
    stateSeting(password, isLeters, isDigits, isSymbols);
  };

  const stateSeting = (pas, lett, didj, symb) => {
    if (pas.length === 0) {
      setColor('gray');
      setRaiting('');
      ...
      setColor('red');
      setRaiting('');
    } else if (lett && symb && didj) {
      setColor('green');
      setRaiting('strong');
    } else if ((lett && symb) || (symb && didj) || (lett && didj)) {
      setColor('yellow');
      setRaiting('medium');
    } else if (didj || lett || symb) {
      setColor('red');
      setRaiting('easy');
    }
  };

  const hendlerCangeParol = event => {
    event.preventDefault();
    if (
    ...
    <>
      <div className={styles.form}>
        ...
        <div>
          ...
          />
          <button className={styles.buttonIcon} onClick={hendlerCangeParol}>
            <svg
        ...
        <div className={styles.raiting}>
          ...
          <div id="strong" className={styles.btn}>
            {raiting === 'strong' ? <span>strong</span> : <></>}
          </div>
        </div>
      </div>
    </>
